...
10. 精度验证：数据集、原始/上游指标、命令、CPU/官方结果、NPU 结果、容差或 delta。
11. 性能验证：命令/工具、warmup、loop、batch/并发、精度模式、latency/FPS/QPS/RTF、芯片。
...
- 精度：尽量使用上游原始指标、数据集 split、预处理、后处理和阈值。若原始指标无法复现，则在相同输入上对比 NPU 与 CPU/upstream baseline，并说明替代原因。
- 性能：优先使用原始项目或同任务 ModelZoo 的可比性能指标。否则按路线采用常用口径：OM 用 `ais_bench` latency/FPS，vLLM 服务用 QPS/tokens/s/latency，音频用 RTF/RTFx，pipeline 同时报告纯模型和端到端 latency。
...
- [ ] CPU/upstream baseline 输出或指标已记录。
