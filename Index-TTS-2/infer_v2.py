        ...
        for sent in segments:
            ...
            with torch.no_grad():
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    ...
                    #print("spk_cond_emb:", spk_cond_emb)
                    #print("text_tokens:", text_tokens)
                    #print("emo_cond_emb:", emo_cond_emb)
                    #print("cond_lengths:", torch.tensor([spk_cond_emb.shape[-1]], device=text_tokens.device))
                    #print("emo_cond_lengths:", torch.tensor([emo_cond_emb.shape[-1]], device=text_tokens.device))
                    #print("emo_vec:", emovec)
...
if __name__ == "__main__":
    ...
    text_zh = '欢迎大家来体验indextts2，并给予我们意见与反馈，谢谢大家。'
    text = 'There is a vehicle arriving in dock number 7?'
    
    words = ['请介绍一下AI算法。', '请介绍一下it公司。', '绝顶一茅茨，直上三十里。', '扣关无僮仆，窥室唯案几']

    tts = IndexTTS2(cfg_path="checkpoints/config.yaml", model_dir="checkpoints", use_cuda_kernel=False)
    
    for word in words:
        tts.infer(spk_audio_prompt=prompt_wav, text=word, output_path="gen.wav", verbose=True)
