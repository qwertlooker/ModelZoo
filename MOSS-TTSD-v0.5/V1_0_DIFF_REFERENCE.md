...
适配含义：v1.0 的 `requirements.txt` 明确绑定 CUDA 版 torch/torchaudio，NPU 环境不能照抄安装。正式文档中应改为 Ascend 兼容 torch/torch-npu/torchaudio 组合，并把 CUDA wheel 约束作为上游 CUDA 参考，不作为 NPU 安装命令。
