...
说明：如果环境中 `torchaudio.load` / `torchaudio.save` 报 `TorchCodec is required for load_with_torchcodec` / `save_with_torchcodec`，请确认已应用最新 patch。本适配不会要求额外安装 `torchcodec`；prompt 音频文件读取与 WAV 写出已改为原依赖中的 `soundfile`，避免 TorchAudio 2.9+ 对 TorchCodec 的强依赖。
