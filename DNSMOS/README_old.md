...
| ------------------ | --------------- | ---------------------- | ----------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  --test_dir     |  -t           |  字符串（目录路径）  |  是             |  -                          |  待评分的WAV音频文件所在目录，脚本会**递归遍历**该目录下的所有.wav文件                                                                                 |
|  --output_csv   |  -o           |  字符串（文件路径）  |  是             |  -                          |  评分结果的输出CSV文件路径，脚本会自动创建父目录
|
|  --personalized  |  -p           |  布尔值（标志位）    |  否             |  False                      |  是否使用**个性化**​**DNSMOS** ​**模型**​： 1）True则加载model_root/pDNSMOS/sig_bak_ovr.onnx； 2）False则加载model_root/DNSMOS/sig_bak_ovr.onnx  |
